...
static gboolean
tracker_db_exec_proc_no_reply (TrackerDBInterface *iface, 
			       const gchar        *procedure, 
			       ...)
{
	TrackerDBResultSet *result_set;
...
static void
save_full_text (TrackerDBInterface *iface,
				     ...
				     GHashTable          *hash_table)
{
	...
	case TRACKER_FIELD_TYPE_FULLTEXT: 
		for (i = 0; i < g_strv_length (values); i++) {
			...
			save_full_text (tracker_db_manager_get_db_interface_content (iface), 
					id, 
					values[i],
					strlen (values[i]));
		}
			     ...
			     const gchar        *search_id)
{
	...
	tracker_db_exec_proc_no_reply (iface, 
...
gchar *
tracker_db_get_id (TrackerDBInterface *iface, 
		   const gchar        *service, 
		   const gchar        *uri)
{
	gint    service_id;
	...
	id = tracker_db_get_file_id (iface, uri);
		      ...
		      const gchar        *moved_to_uri)
{
	...
	id = tracker_db_get_file_id (iface, moved_from_uri);
...
static gchar *
str_get_after_prefix (const gchar *source,
		      const gchar *delimiter)
{
	gchar *prefix_start, *str;
